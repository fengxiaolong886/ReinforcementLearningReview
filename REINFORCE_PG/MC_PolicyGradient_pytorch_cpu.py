...
#        tensor_actions = torch.Tensor(self.episode_actions).to(self.device)
        tensor_rewards = torch.Tensor(discounted_episode_reward_normalized).to(self.device)
#        action_onehot_ = np_utils.to_categorical(self.episode_actions,
#                                               num_classes=self.n_action)
#        action_onehot = torch.Tensor(action_onehot_).to(self.device)
...
#        action_one_hot = torch.zeros(len(self.episode_actions),
#                                     self.n_action).scatter_(1, tensor_actions, 1)
